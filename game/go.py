...
#-----------------------ENV code fo the game-------------------------#


BOARD_SIZE = 20  # number of rows/cols = BOARD_SIZE - 1


def opponent_color(my_col):
    if my_col == 'WHITE':
        return 'BLACK'
    elif my_col == 'BLACK':
        return 'WHITE'
    else:
        print('Invalid color: ' + my_col)
        return KeyError


def neighbors(point): #returns neighboring points
    neighboring = [(point[0] - 1, point[1]), # coords are calcualted as such (x-1,y),(x+1,y),(x,y-1),(x,y+y)
                   (point[0] + 1, point[1]),
                   (point[0], point[1] - 1),
                   (point[0], point[1] + 1)]
...
def calculate_liberty(points, board):
    """Find and return the liberties of the point."""
    #checking unoccupied liberrty points currently acc too black and white grps
    librties = [point for point in neighbors(points)
                 if not board.stonedict.get_groups('BLACK', point) and not board.stonedict.get_groups('WHITE', point)]
    return set(librties)


class Group(object): #class for when a groupof stones is made
    ...
    def add_stones(self, pointlist):#only updates stones
...
class Board(object):
    ...
    def create_group(self, point, color):
        # Update group list
        ll = calculate_liberty(point, self)
    ...
    def merge_groups(self, grouplist, point):
        ...
        all_liberties = all_liberties | calculate_liberty(point, self)
    ...
    def _get_legal_actions(self):
        ...
        for action in legal_actions:
            if len(calculate_liberty(action, self)) > 0:
    ...
    def shorten_liberty_for_groups(self, point, color):   
        # Removes the liberty from all belonging groups.
        ...
        # Checking the  opponent's groups first
   ...
   #   To see if a stone has been placed on the boar
